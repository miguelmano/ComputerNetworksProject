...
void TCP_reply(bool there_is_file, string & reply_first_part, ifstream & file, string file_name, int & fd, int & new_fd, struct sockaddr_in & addr, socklen_t & addrlen, struct addrinfo* res)
{
    int n_written;
    char buffer[1];

    // write reply_first_part
    int bytes_to_write = reply_first_part.length();
    char* c_reply = turn_into_cstring(reply_first_part);
    char* ptr = c_reply;
    ...
    if (there_is_file)
    {
        bytes_to_write = file_size(file_name);

        while (bytes_to_write > 0)
        {
            file.read(buffer, 1);
            n_written = write(new_fd, buffer, 1);
            if (n_written <= 0) report_error_and_exit("Error: TCP: Failed to sent part of the reply to client.");
            bytes_to_write -= 1;
        }
    }
...
{     
    ...
    else if (argc == 4) // specified [-p GS_port] and [-v]
    {
        string p_flag = argv[2];
        ...
        {   
            ...
            switch (get_option(parsed_msg[0]))
            {
                ...
                case 1: // <play or pl>; receive: "PLG <PLID> <letter> <trial>\n"; reply: "RLG <status> <trial> [n pos*]\n"
                {
                    ...
                    // error check
                    /* if (parsed_line.size()!=2 || isInvalidPLID(PLID)==1 || wrongSyntax) */
                    if (false) // status ERR
                    {
    ...
    else if (pid  > 0) // TCP  process
    {
        ...
        while (true)
        {
            ...
            // receive client's message
            string recvd_msg = TCP_receive(fd, new_fd, addr, addrlen, hints, res);
            vector<string> parsed_msg = split_string(recvd_msg);
            ...
            switch (get_option(parsed_msg[0]))
            {
                case 5: // "<state or st>; receive: "STA <PLID>\n"; reply: "RST <status> [Fname Fsize Fdata]\n"
                {
                    ...
                    if (has_ongoing_game) // status ACT
                    {
                        ...
                        string reply = "RST ACT " + Fname + " " + Fsize + " ";
                        TCP_reply(true, reply, file, file_name, fd, new_fd, addr, addrlen, res);
                    }
                    else
                    {
                        if (file_or_dir_exists(PLID)) // status FIN
                        {
                            ...
                            recent_file.close();
                            string reply = "RST FIN " + Fname + " " + Fsize + " ";
                            
                            // send reply
                            TCP_reply(true, reply, recent_file, Fname, fd, new_fd, addr, addrlen, res);
                        }
                        else // status NOK
                        {
                            string reply = "RST NOK";
                            TCP_reply(false, reply, file, NULL, fd, new_fd, addr, addrlen, res);
                        }
                    }
