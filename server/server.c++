...
bool is_valid_port(string GS_port)
{
    if (GS_port.size() != 5) // number of chars
        return false;

    for (char c : GS_port) // all chars are numbers
...
void end_game(string result, string PLID, string file_name)
{
    ...
    {
        const char* dir = turn_into_c_string(PLID);
        mkdir(dir, 0777);
    }
...
string get_last_file_in_dir(string dir_name)
{
    DIR *d;
    struct dirent *dir;
...
void UDP_reply(string reply, int & fd, struct sockaddr_in & addr, socklen_t & addrlen, struct addrinfo* & res)
{
    ...
    // reply to client
    n_written = sendto(fd, turn_into_c_string(reply), reply.length(), 0, (struct sockaddr*) &addr, addrlen);
...
{                      
    ...
    char* c_reply = turn_into_c_string(reply);
    char* ptr = &c_reply[0];
...
{     
    ...
    else if (argc == 3) // specified [-v]
    {
        string v_flag = argv[2];
        if (v_flag != "-v") report_error_and_exit("Error: You must follow the correct syntax: ./server <word_file> [-p GS_port] [-v]");
    ...
    else if (argc == 4) // specified [-p GS_port]
    {
        ...
        if (p_flag != "-p") report_error_and_exit("Error: You must follow the correct syntax: ./server <word_file> [-p GS_port] [-v]");
        GS_port = argv[3];
        if (!is_valid_port(GS_port)) report_error_and_exit("Error: You must follow the correct syntax: ./server <word_file> [-p GS_port] [-v]");
    }
    else if (argc == 5) // specified both [-p GS_port] and [-v]
    {
        string flag = argv[2];
        if (flag == "-p")
        {
            ...
            string other_flag = argv[4];
            if (other_flag == "-v") verbose_mode_enabled = true;
            else report_error_and_exit("Error: You must follow the correct syntax: ./server <word_file> [-p GS_port] [-v]");
        }
        else if (flag == "-v")
        {
            verbose_mode_enabled = true;

            string other_flag = argv[3];
            if (other_flag == "-p")
            {
                GS_port = argv[3];
                if (!is_valid_port(GS_port)) report_error_and_exit("Error: You must follow the correct syntax: ./server <word_file> [-p GS_port] [-v]");
        ...
        {
            report_error_and_exit("Error: You must follow the correct syntax: ./server <word_file> [-p GS_port] [-v]");
        }
    }
    ...
    {    
        ...
        {   
            ...
            {
                cout << "New UDP connection to player with PLID " + parsed_msg[1] + ": ";
                cout << recvd_msg << endl;
            }
            
            switch (get_case(parsed_msg[0]))
            {
                case 0: // <start or st>; receive: "SNG <PLID>\n"; reply: "RSG <status> [n_letters max_errors]"
                {
                    ...
                    // error check
                    if (parsed_msg.size() != 2 || !PLID_is_valid(PLID))
                    {
                ...
                case 1: // <play or pl>; receive: "PLG <PLID> <letter> <trial>\n"; reply: "RLG <status> <trial> [n pos*]\n"
                {
                    ...
                    // error check
                    if (parsed_msg.size() != 4 || !PLID_is_valid(PLID)) // status ERR
                    {
                ...
                case 6: // <quit or exit>; receive: "QUT <PLID>\n"; reply: "RQT <status\n"
                {
                    ...
                    // error check
                    if (parsed_msg.size() != 2 || !PLID_is_valid(PLID)) // status ERR
                    {
    ...
    else if (pid  > 0) // TCP  process
    {
        ...
        while (true)
        {
            ...
            {   
                ...
                string trimmed_recvd_msg = trim_until_new_line(recvd_msg);
                vector<string> parsed_msg = split_string(trimmed_recvd_msg);
                
                if (verbose_mode_enabled)
                {
                    cout << "New TCP connection to player with PLID " + parsed_msg[1] + ": ";
                    cout << trimmed_recvd_msg << endl;
                }
                    ...
                    case 5: // "<state or st>; receive: "STA <PLID>\n"; reply: "RST <status> [Fname Fsize Fdata]\n"
                    {
                        ...
                        else
                        {
                            if (file_or_dir_exists(PLID)) // status FIN
                            {
                                ...
                                string file_name = get_last_file_in_dir(PLID);
                                string file_path = PLID + "/" + file_name;
